...
export class CartsController extends CrudController<ICart> {
    constructor(private readonly cartsService: CartsService<ICart>) {
        super(cartsService)
    }

    /*
        Route:        GET api/cart/chunk
        Roles:        user, admin
    ...
    */
    ...
    async chunkData(@Query() query: {}, @Res() res: Response, @Req() req: Request) {
        ...
        const totalRecords = await this.cartsService.count(query);
        filter.chunks = totalRecords / 3000;
        filter.limit = 3000;

        for (let i = 0; i < filter.chunks; i++) {
            let results = await this.cartsService.findAllPaginated(query, filter);
